...
public class PdfGeneratorService {
    ...
    public String generateQcmResultsPdf(Qcm qcm, List<QcmAttempt> attempts, Map<Long, String> learnerNames) throws IOException {
        String pdfDir = storageDir + "/qcms/results";
        new File(pdfDir).mkdirs();
        String filename = "qcm_" + qcm.getId() + "_results.pdf";
        String fullPath = pdfDir + "/" + filename;
        ...
        Long levelId = qcm.getLevelId() != null ? qcm.getLevelId() : 1L;
        String levelCode = LEVEL_CODES.getOrDefault(levelId, "?");

        // Header
        doc.add(new Paragraph("SprachReise — Résultats QCM")
                .setFont(fontBold).setFontSize(20).setFontColor(C_DEEP).setMarginBottom(4));
        doc.add(new Paragraph(qcm.getTitle() + "  ·  Niveau " + levelCode)
                .setFont(fontOblique).setFontSize(12).setFontColor(C_ACCENT).setMarginBottom(16));

        DateTimeFormatter fmt = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");

        // Table
        Table table = new Table(UnitValue.createPercentArray(new float[]{15, 35, 20, 30}))
                .useAllAvailableWidth();
        String[] headers = {"Apprenant #", "Nom", "Score", "Date"};
        for (String h : headers) {
            table.addHeaderCell(new Cell()
                    .setBackgroundColor(C_DEEP)
                    .setBorder(new SolidBorder(C_ACCENT, 0.5f))
                    .setPadding(8)
                    .add(new Paragraph(h).setFont(fontBold).setFontSize(10).setFontColor(C_PARCH)));
        }
        boolean alt = false;
        if (attempts != null) {
            for (QcmAttempt at : attempts) {
                DeviceRgb bg = alt ? new DeviceRgb(245, 239, 227) : C_WHITE;
                String name = learnerNames != null
                        ? learnerNames.getOrDefault(at.getLearnerId(), "Apprenant #" + at.getLearnerId())
                        : "Apprenant #" + at.getLearnerId();
                String scoreStr = at.getScore() != null ? at.getScore().toString() : "-";
                if (at.getTotalQuestions() != null) scoreStr += " / " + at.getTotalQuestions();
                String dateStr = at.getAttemptedAt() != null ? at.getAttemptedAt().format(fmt) : "-";

                table.addCell(rowCell(String.valueOf(at.getLearnerId()), fontRegular, bg));
                table.addCell(rowCell(name, fontRegular, bg));
                table.addCell(rowCell(scoreStr, fontBold, bg));
                table.addCell(rowCell(dateStr, fontRegular, bg));
                alt = !alt;
            }
        }
        doc.add(table);

        int total = attempts != null ? attempts.size() : 0;
        doc.add(new Paragraph("\nTotal : " + total + " tentative(s)")
                .setFont(fontOblique).setFontSize(10).setFontColor(C_MUTED));

        doc.close();
        return "qcms/results/" + filename;
    }

    private Cell rowCell(String value, PdfFont font, DeviceRgb bg) {
        return new Cell().setBackgroundColor(bg)
                .setBorder(new SolidBorder(C_CREAM, 0.5f))
                .setPadding(6)
                .add(new Paragraph(value).setFont(font).setFontSize(9).setFontColor(C_DEEP));
    }
